...
const Project = () => {
    ...
    const fetchProject = async (projectID) => {
      try {
        const response = await fetch(
          `http://127.0.0.1:8000/api/project-detail/${projectID}`
        );
  ...
  async function handleRun() {
    ...
    if (response.ok) {
      ...
      //let out = await JSON.parse(serverResponse.output);
      console.log(serverResponse);
      console.log(typeof serverResponse);
      //console.log(out);
      setAns(serverResponse);
    } else {
      console.log("error");
    }
  }
  ...
  async function saveProject() {
    if (typeof projectID === "undefined") {
      ...
      const response = await fetch(
        "http://127.0.0.1:8000/api/register-project",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/api/update-project/${projectID}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    <>
      ...
      <div style={{ display: "flex", height: "100vh" }}>
        ...
        <div style={{ flex: 1, padding: "20px", position: "relative" }}>
          ...
          <div
            className="box"
            style={{ position: "absolute", top: "30px", right: "27px" }}
          >
            <textarea
              value={projectName}
